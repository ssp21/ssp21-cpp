...
TEST_CASE(SUITE("sha256"))
{
    ...
    auto hex = to_hex(output.as_seq(), false);
...
TEST_CASE(SUITE("HKDF-sha256"))
{
    ...
    Crypto::hkdf_sha256(seq32_t(salt, 8), { ikm }, key1, key2);

    REQUIRE(key1.get_type() == BufferType::symmetric_key);
    REQUIRE(key2.get_type() == BufferType::symmetric_key);

    auto hex_key_1 = to_hex(key1.as_seq(), false);
    auto hex_key_2 = to_hex(key2.as_seq(), false);
