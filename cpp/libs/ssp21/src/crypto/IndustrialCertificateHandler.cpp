...
CertificateBody IndustrialCertificateHandler::read_anchor_cert(const seq32_t& file_data)
{
    ContainerFile file;
    {
        ...
        if (any(err))
            throw Exception("Unable to read container file: ", ParseErrorSpec::to_string(err));
    }
    ...
    CertificateChain chain;
    {
        ...
        if (any(err))
            throw Exception("Unable to read certificate chain: ", ParseErrorSpec::to_string(err));
    }

    if (chain.certificates.count() != 1)
        throw Exception("Unexpected number of certificates: ", chain.certificates.count());

    CertificateBody body;
    const auto err = body.read_all(chain.certificates.get(0)->certificate_body);
    if (any(err))
        throw Exception("Unable to read certificate body: ", ParseErrorSpec::to_string(err));
...
{
    if (mode != HandshakeMode::industrial_certificates)
