...
TEST_CASE(SUITE("sha256"))
{
    ...
    auto hex = to_hex(output.as_slice().widen<uint32_t>(), false);
...
TEST_CASE(SUITE("sha256 can safely be invoked on its own output buffer"))
{
    ...
    // now mix the hash
    Crypto::hash_sha256({ output.as_slice().widen<uint32_t>(), slice }, output);
...
TEST_CASE(SUITE("HKDF-sha256"))
{
    ...
    auto hex_key_1 = to_hex(key1.as_slice().widen<uint32_t>(), false);
    auto hex_key_2 = to_hex(key2.as_slice().widen<uint32_t>(), false);
