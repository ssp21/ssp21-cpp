...
object EnumFromType extends HeaderImplModelRender[EnumModel] {
  ...
  private def signature(enum: EnumModel) = List("static", enum.name, List("from_type(", enum.enumType.cppType," arg)").mkString).mkString(" ")

  private def implSignature(enum: EnumModel) = List(enum.name, List(enum.specName,"::from_type(", enum.enumType.cppType," arg)").mkString).mkString(" ")
